...
export async function POST(request) {
  ...
  try {
    ...
    for (const customer of customers) {
      try {
        const emailHtml = generateEmailHtml(customer, campaign);
        await sendCampaignEmail(customer.email, emailSubject, emailHtml);
        results.sent++;
        ...
        if (error.responseCode === 550 && error.response.includes('Too many emails')) {
...
function generateEmailHtml(customer, campaign) {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h1 style="color: #2563eb;">${campaign.campaignName}</h1>
      <p>Hi ${customer.name},</p>
      <p>We're excited to offer you ${campaign.discountPercent}% off your next purchase!</p>
      <div style="background-color: #f0f9ff; border: 1px dashed #2563eb; padding: 15px; margin: 15px 0; text-align: center;">
        <p style="font-size: 18px; font-weight: bold;">Use promo code: ${campaign.promoCode}</p>
      </div>
      <p>Valid from ${new Date(campaign.startDate).toLocaleDateString()} to ${new Date(campaign.endDate).toLocaleDateString()}</p>
      <p>Thank you for being a valued customer!</p>
      <p style="margin-top: 30px; font-size: 12px; color: #666; text-align: center;">
        © ${new Date().getFullYear()} eBikri. All rights reserved.<br>
